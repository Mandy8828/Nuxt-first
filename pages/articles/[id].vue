<template>
  <div class="flex w-full justify-center px-6 lg:px-0">
    ...
    <template v-else>
      ...
      <div
        v-else-if="article"
        class="mb-8 flex w-full flex-col justify-center md:max-w-3xl"
      >
        <div class="mt-4 flex justify-center">
          <img :src="article.cover" />
        </div>
        <div
          class="my-2 flex flex-col justify-between sm:my-0 sm:flex-row sm:items-center"
        >
        ...
        <h1 class="break-words text-4xl font-semibold text-gray-700">
          {{ article.title }}
        </h1>
...
const {
  ...
  data: article,
  error,
} = await useFetch(`/api/articles/${route.params.id}`);
